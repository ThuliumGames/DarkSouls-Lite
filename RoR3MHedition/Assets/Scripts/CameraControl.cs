...
public class CameraControl : MonoBehaviour {
	...
	public float lerp, rotSpeed, maxDistance;
	...
	void FixedUpdate () {
		...
		vector = Vector3.Lerp(vector, point, (lerp*0.02f)+((Vector3.Dot(transform.forward, (new Vector3 (vector.x, 0, vector.z)-new Vector3 (objToFollow.position.x, 0, objToFollow.position.z)).normalized)+1)/2.0f)+(Mathf.Abs(ang)/89.0f));
		
		//Undo Rotation
		transform.RotateAround(point, transform.right, -ang);
		
		//Move to follow location
		transform.position = vector;
		
		//Set up down angle
		ang = Mathf.Clamp(ang-Input.GetAxis("Mouse Y")*rotSpeed*0.02f, -89, 89);
		transform.eulerAngles = new Vector3(ang, transform.eulerAngles.y, 0);
		
		//Set Left Right angle
		transform.RotateAround(transform.position, Vector3.up, Input.GetAxis("Mouse X")*rotSpeed*0.02f);
		
		//Test for wall
		RaycastHit h;
		Physics.BoxCast(transform.position, Vector3.one*0.5f, -transform.forward, out h, Quaternion.Euler(Vector3.zero), maxDistance, lm);
	...
	void Update () {
		if (Globals.animationSpeed < 1) {
			cam.position = cam.position + Random.insideUnitSphere * 0.1f;
		}
	}
}
